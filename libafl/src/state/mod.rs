...
/// symimqueue
const MAX_LENFORSYMIMQUEUE: usize = 8;
...
impl PathTracker {
    ...
    ///
    pub fn path_hits(&self) -> &Vec<u32>{
        &self.path_hits
    }
    ///
    pub fn path_hits_mut(&mut self) -> &mut Vec<u32>{
        &mut self.path_hits
    }
    ///
    ...
    ///
    pub fn stuck_paths(&self) -> &usize {
        &self.stuck_paths
    }
    ///
    pub fn stuck_paths_mut(&mut self) -> &mut usize {
        &mut self.stuck_paths
    }
    /// last cycle
    pub fn last_cycle(&self) -> &u64{
        &self.last_cycle
    }
    /// when cycle+1 reset the pathtracker
    pub fn cycle_reset(&mut self) {
        self.path_hits = vec![0; N_FUZZ_SIZE];
...
pub struct StdState<C, I, R, SC> {
    ...
    is_symcompleted:bool,
    path_tracker:PathTracker,
    symim_queue:Vec<CorpusId>,

    phantom: PhantomData<I>,
}

/// is_symcompleted
pub trait HasSymCompleted {
    /// is_symcompleted
    fn is_symcompleted(&self) -> &bool;
    /// is_symcompleted mut
    fn is_symcompleted_mut(&mut self) -> &mut bool;
}
impl<C, I, R, SC> HasSymCompleted for StdState<C, I, R, SC>{
    /// is_symcompleted
    fn is_symcompleted(&self) -> &bool{
        &self.is_symcompleted
    }
    /// is_symcompleted mut
    fn is_symcompleted_mut(&mut self) -> &mut bool{
        &mut self.is_symcompleted
    }
} 

/// executedbmp for symcc

pub trait HasPathTracker{
    /// get executed_bmp
    fn path_tracker(&self) -> &PathTracker;

    /// get executed_bmp muted
    fn path_tracker_mut(&mut self) -> &mut PathTracker;
}
impl<C, I, R, SC> HasPathTracker for StdState<C, I, R, SC>{
    /// get executed_bmp
    fn path_tracker(&self) -> &PathTracker{
        &self.path_tracker
    }
...
/// symim
pub trait HasSymimQueue {
    /// queue
    fn symim_queue(&self) -> &Vec<CorpusId>;
    /// queue mutable
    fn symim_queue_mut(&mut self) -> &mut Vec<CorpusId>;
    /// push 
    fn symim_push(&mut self, id:CorpusId);
    /// pop
    fn symim_pop(&mut self) -> Option<CorpusId>;
    /// full
    fn is_symfull(&mut self) -> bool;
}

impl<C, I, R, SC> HasSymimQueue for StdState<C, I, R, SC> 
where
    I: Serialize + DeserializeOwned
{
    ...
    fn symim_queue_mut(&mut self) -> &mut Vec<CorpusId> {
        &mut self.symim_queue
    }

    fn symim_push(&mut self, id: CorpusId) {
        if self.symim_queue().len() >= MAX_LENFORSYMIMQUEUE {
            self.symim_queue_mut().remove(0); 
        }
        self.symim_queue_mut().push(id); 
    }

    fn symim_pop(&mut self) -> Option<CorpusId> {
        if !self.symim_queue().is_empty() {
            Some(self.symim_queue_mut().remove(0)) 
        } else {
            None  
        }
    }
    ...
    fn is_symfull(&mut self) -> bool{
        self.symim_queue().len() == MAX_LENFORSYMIMQUEUE
    }
}
    ...
    SC: Corpus<I>,
{
        ...
        SC: Serialize + DeserializeOwned,
    {
        let mut state = Self {
            ...
            is_symcompleted:true,
            path_tracker:PathTracker::new(3),
            //executed_bmp:vec![false;N_FUZZ_SIZE],
            symim_queue:Vec::with_capacity(MAX_LENFORSYMIMQUEUE),
            phantom: PhantomData,
            #[cfg(feature = "std")]
